...
begin
  ...
  if v_transfer_id is null then
    ...
    insert into public.transfer_counts (transfer_id, case_item_id, stage, counted_quantity)
      select v_transfer_id, id, 'checkout', current_quantity from public.case_items where suitcase_id = suitcase_record.id;
  end if;
  ...
  for count_entry in select value from jsonb_array_elements(p_counts) loop
    ...
    select v_transfer_id, ci.id, 'takeover', greatest(0, (count_entry ->> 'counted_quantity')::integer),
  ...
  select exists (
    ...
    where takeover.transfer_id = v_transfer_id and takeover.stage = 'takeover'
  ...
  update public.case_items ci set current_quantity = counts.counted_quantity
    ...
    where counts.transfer_id = v_transfer_id and counts.stage = 'takeover' and counts.case_item_id = ci.id;
  ...
  values (suitcase_record.id, v_transfer_id,
